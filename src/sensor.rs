...
pub trait Sensor {
    ...
    fn insert(&mut self, item: &dyn SensorData) -> Rc<RefCell<dyn Neuron>>;
    
    fn search(&self, item: &dyn SensorData) -> Option<Rc<RefCell<dyn Neuron>>>;
    ...
    fn deactivate(
        &mut self, item: &dyn SensorData, propagate_horizontal: bool, propagate_vertical: bool
    ) -> Result<(), String>;
